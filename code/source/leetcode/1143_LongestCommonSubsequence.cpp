 ...
 * 1143. 最长公共子序列
 * 
 * 给定两个字符串 text1 和 text2，返回这两个字符串的最长公共子序列的长度。如果不存
 * 在公共子序列，返回 0。
 * 
 * 一个字符串的子序列是指这样一个新的字符串：它是由原字符串在不改变字符的相对顺序的
...
public:
     ...
     * 方法一：动态规划
     * 
     * 假设 text1 和 test2 的长度为 m 和 n，创建 m+1 行 n+1 列的二维数组 dp，其中
     * dp[i][j] 表示 text1[0:i] 和 text2[0:j] 的最长公共子序列的长度。
     * 
     * 上述表示中，text1[0:i] 表示 text1 的长度为 i 的前缀，text2[0:j] 表示 text2
     * 的长度为 j 的前缀。
     * 
     * 考虑动态规划的边界情况：
     *  - 当 i = 0 时，text1[0:i] 为空，空字符串和任意字符串的最长公共子序列的长度
     *    都是 0，因此，对于任意 0 <= j <= n，有 dp[0][j] = 0。
     *  - 当 j = 0 时，text2[0:j] 为空，同理可得，对任意 0 <= i <= m，有
     *    dp[i][0] = 0。
     * 因此动态规划的边界是：当 i = 0 或 j = 0 时，有 dp[i][j] = 0。
     * 
     * 当 i > 0 且 j > 0 时，考虑 dp[i][j] 的计算：
     *  - 当 text1[i-1] 等于 text2[j-1] 时，将这两个相同的字符称为公共字符，考虑
     *    text1[0:i-1] 和 text2[0:j-1] 的最长公共子序列，再增加一个字符（即公共字
     *    符）即可得到 text1[0:i] 和 text2[0:j] 的最长公共子序列，因此
     *    dp[i][j] = dp[i-1][j-1] + 1。
     *  - 当 text1[i-1] 不等于 text2[j-1] 时，dp[i][j] 应取 dp[i-1][j] 和 
     *    dp[i][j-1] 中较大的一项。
     * 因此，可得到状态转移方程：
     *             0                            i = 0 或 j = 0
...
TEST(Solution, longestCommonSubsequence) {
    ...
    EXPECT_EQ(0, s.longestCommonSubsequence("", ""));
