/**
 * Source : https://leetcode-cn.com/problems/decode-ways-ii/
 * Author : Hu Jingbo
 * Date   : 2021-10-04
 * 
 * 639. 解码方法 II
 ...
 * "06" 不同。
 * 
 * 除了上面描述的数字字母映射方案，编码消息中可能包含 '*' 字符，可以表示从 '1' 到 '9'
 * 的任一数字（不包括 '0'）。例如，编码字符串 "1*" 可以表示 "11"、"12"、"13"、"14"、
 * "15"、"16"、"17"、"18" 或 "19" 中的任意一条消息。对 "1*" 进行解码，相当于解码该字
 * 符串可以表示的任何编码消息。
 * 
...
public:
     ...
     * 方法一：动态规划
     * 
     * 对于给定字符串 s，设它的长度是 n，其中的字符从左到右依次是 s[1], s[2], ... ,
     * s[n]。我们可以使用动态规划的方法计算出字符串 s 的解码方法数。
     * 
     * 具体地，设 f[i] 表示字符串 s 的前 i 个字符的解码方法数。在进行状态转移时，我
     * 们考虑最后一次解码使用了 s 中的哪些字符，那么会有下面的两种情况：
     ...
     *    - 如果 s[i-1] 和 s[i] 均为 *，那么它们对应这 [11, 19] 以及 [21, 26] 中
     *      任意一种编码，共有 15 种方案。因此
     *      f[i] = 15 * f[i-2]
     *    - 如果 s[i-1] 为 *，当 0 <= s[i] <= 6 时，s[i-1] 可以选择 1 和 2；当
     *      7 <= s[i] <= 9 时，s[i-1] 只能选择 1。因此
     *      f[i] = 2 * f[i-2],       0 <= s[i-1] <= 6
     *             f[i-2],           7 <= s[i-1] <= 9
     *    - 如果 s[i] 为 *，当 s[i-1] = 1 时，s[i] 可以在 [1, 9] 中选择；当 s[i-1]
     *      为 2 时，s[i] 可以在 [1, 6] 中选择；对于其它情况，它们无法解码，因此
     *             9 * f[i-2]       s[i] = 1
     *      f[i] = 6 * f[i-2]       s[i] = 2
     *             0                其它
     *    - 如果 s[i-1] 和 s[i] 均不为 *，那么只有 s[i-1] 不为 0 并且 s[i-1] 和
     *      s[i] 组成的数字小于等于 26 时，它们才能被解码。因此
...
TEST(Solution, numDecodings) {
    ...
    EXPECT_EQ(1, s.numDecodings(""));
    EXPECT_EQ(9, s.numDecodings("*"));
    EXPECT_EQ(18, s.numDecodings("1*"));
    EXPECT_EQ(15, s.numDecodings("2*"));
}