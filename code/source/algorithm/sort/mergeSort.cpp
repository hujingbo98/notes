...
public:
     ...
     * 
     * 当待排序的序列长度为 1 时，递归开始回升，在这种情况下不需要做任何工作，因为长度
     * 为 1 的序列都已排好序。
     * 
     * 时间复杂度是 O(nlogn)，其中 n 是序列的大小。归并排序的时间复杂度与输入的数据无
     * 关，最好和最坏情况都是 O(nlogn)。
     * 
...
TEST(Algorithm, mergeSort) {
    ...
    m.mergeSort_recursive(nums);
    int n = nums.size();
    for (int i = 0; i < n; ++i) {
        ASSERT_EQ(i, nums[i]);
    }

    vector<int> nums_2 = {5, 4, 2, 3, 1, 0};
    m.mergeSort_iterative(nums_2);
    n = nums_2.size();
    for (int i = 0; i < n; ++i) {
        ASSERT_EQ(i, nums_2[i]);
    }
}