...
hashtable 会有无法处理的类型，这就需要自己实现 hash 函数。

在实际开发中，如果需要排序功能，则只能使用 set。

在数据量不大的情况下，哈希表的常数时间复杂度会大于红黑树的 logn，此时应使用 set。

在海量数据的情况下，红黑树的性能会差很多，这取决于红黑树的高度，此时应该使用 unordered_set 来提高性能。空间方面，红黑树会保存一些红黑树的属性和节点指针，好处是不会有未使用的空间浪费，也不会重新分配空间。hashtable 底层采用数组保存，一般数组的大小是大于数据大小的，使用链表来解决哈希冲突，如果冲突过多 (8 个以上) 也会使用 RB-Tree 来提高性能，这将导致 hashtable 有一定的未使用空间浪费。并且 hashtable 会有扩容机制，扩容会导致重新分配内存，然后 rehash，如果海量数据，这里的成本将会很高，所以如果提前大概知道 hashtable 的大小，可以使用 reserve 来提前分配空间，将会避免 rehash。
