...
模板并不创建任何函数和类，而只是告诉编译器该如何定义函数。当我们使用模板时，编译器会推断出模板的实参，编译器使用推断出的参数，实例化一个特定版本的类或函数。
...
在定义类时，应该为该类添加移动构造和移动赋值，如果该类的对象保存在 STL 容器中，在容器发生空间重新配置时，会优先调用移动构造，这将会大大提高性能。
