...
TCP 的发送方使用滑动窗口机制来实现流量控制，滑动窗口大小不能超过对方的接收窗口大小。滑动窗口的起始位置是发送并被确认的数据的下一个字节，当数据被发送和确认时，滑动窗口左边沿向右边沿滑动，称为窗口合拢。当另一端的接收进程读取已经确认的数据并释放了 TCP 接收缓存时，窗口的右边沿向右滑动将允许发送更多的数据，称为窗口张开。
