 ...
 * 212. 单词搜索 II
 * 
 * 给定一个 m x n 二维字符网格 board 和一个单词（字符串）列表 words，找出所有同时
 * 在二维网格和字典中出现的单词。
 * 
 * 单词必须按照字母顺序，通过相邻的单元格内的字母构成，其中“相邻”单元格是那些水平
 * 相邻或垂直相邻的单元格。同一个单元格内的字母在一个单词中不允许被重复使用。
 * 
 * 示例 1：
 * 输入：
 *      board = [["o","a","a","n"],
...
TEST(Solution, findWords) {
    ...
    vector<string> res_2 = s.findWords(board_2, words_2);
    EXPECT_EQ(0, res_2.size());

    vector<vector<char>> board_3;
    vector<string> words_3;
    vector<string> res_3 = s.findWords(board_2, words_2);
    EXPECT_EQ(0, res_3.size());
}