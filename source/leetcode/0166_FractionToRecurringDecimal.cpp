/**
 * Source : https://leetcode-cn.com/problems/fraction-to-recurring-decimal/
 * Author : Hu Jingbo
 * Date   : 2021-10-03
 * 
 * 166. 分数到小数
 * 
 * 给定两个整数，分别表示分数的分子 numerator 和分母 denominator，以字符串形式返回小
 * 数。
 * 
...
public:
     ...
     * 方法一：长除法 + 哈希表
     * 
     * 将分数转换成整数或小数，做法是计算分子和分母相除的结果。可能的结果有三种，整数、
     * 有限小数、无限循环小数。
     * 
     * 如果分子可以被分母整除，则结果是整数，将分子除以分母的商以字符串形式返回即可。
     * 
     * 如果分子不能被分母整除，则结果是有限小数或无限循环小数，需要通过模拟长除法的方
     * 式计算结果。为了方便处理，首先根据分子和分母的正负确定答案的正负（注意此时分子
     * 和分母都不为 0），然后将分子和分母都转成正数，再计算长除法。
     * 
     ...
     * 完成上述拼接之后，根据余数计算小数部分。
     * 
     * 计算小数部分时，每次将余数乘以 10，然后计算小数的下一位数字，并得到新的余数。重
     * 复上述操作知道余数为 0 或找到循环节。
     *  - 如果余数为 0，则结果是有限小数。
     *  - 如果找到循环节，则在循环节的开始位置和结束位置插入括号。
     * 
     * 如何找到循环节？ 对于相同的余数，得到的小数的下一位数字一定是相同的，因此如果
     * 计算过程中发现某一位的余数在之前已经出现过，则为找到循环节。为了记录每个余数是否
     * 出现过，需要使用哈希表存储每个余数在小数部分第一次出现的下标。
     * 
     * 在每次得到余数时，判断该余数之前是否出现过，如果出现过则在第一次出现的位置加上
     * 左括号，在末尾加上右括号返回即可。
     * 
...
TEST(Solution, fractionToDecimal) {
    ...
    EXPECT_EQ("0", s.fractionToDecimal(0, 2));
    EXPECT_EQ("0", s.fractionToDecimal(0, -2));
    EXPECT_EQ("0.5", s.fractionToDecimal(1, 2));
    EXPECT_EQ("-0.5", s.fractionToDecimal(-1, 2));
    EXPECT_EQ("2", s.fractionToDecimal(2, 1));
    EXPECT_EQ("0.(6)", s.fractionToDecimal(2, 3));
    EXPECT_EQ("0.(012)", s.fractionToDecimal(4, 333));
    EXPECT_EQ("0.2", s.fractionToDecimal(1, 5));
}