 ...
 * 它们的顺序错误就把它们交换过来。
 * 
 * 具体做法是，按顺序遍历 [a[0], a[n])。在遍历 a[i] 的过程中，比较 a[i] 和 a[i+1]，
 * 如果 a[i] 大于 a[i+1]，就交换它们两个。当遍历完成后，最大的元素就会冒到数列的最
 * 后。然后我们将遍历的范围缩小到 [a[0], a[n-1])。继续遍历，直至遍历的范围长度缩小
 * 到 1，则排序完成。
 * 
...
TEST(Algorithm, bubbleSort_V2) {
    vector<int> nums = {5, 4, 2, 3, 1, 0};
    nums = bubbleSort_V2(nums);
    int n = nums.size();
    for (int i = 0; i < n - 1; ++i) {
        EXPECT_TRUE(nums[i] < nums[i+1]);
    }
